...
const renewQuestion = (randomizing) => {
    ...
    options.forEach(e => {
        ...
        e.classList.remove("btn-good")
        e.classList.remove("btn-bad")
    })
...
options.forEach(x => x.addEventListener("click", e => {
    ...
    if (`opt${e.target.dataset.num}` == vyber.correct) {
        e.target.classList.add("btn-good")
        score += 10;
    }
    else {
        e.target.classList.add("btn-bad")
    }
...
dis_btn.addEventListener("click", () => {
    ...
    if (otazkyCount >= otazkyCelkem) {
        ...
        if (score > localStorage.getItem("highscore")) {
            ...
            console.log(highscore.getItem("highscore"))
        }
        // end screen
        prog.remove()
